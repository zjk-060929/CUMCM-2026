...
def figure_simulation() -> None:
    ...
    axes[0].axvline(
        ...
        linewidth=1.7,
        linestyle=":",
        label=f"名义一阶最坏值 {theoretical:.2f} m",
    )
    ...
    axes[1].set_title("(b) 自设分布下最小包围圆半径的经验分布")
...
def figure_deterministic_checks() -> None:
    """绘制边界反例、近距离形状和同场景候选点对照。"""
    ...
    boundary_reading = (
        bearing_deg(second, boundary_source) + ANGLE_ERROR_DEG
    ) % 360.0
    boundary_result = solve_localization(
        [
            Measurement(0.0, 0.0, 0.0),
            Measurement(*second, boundary_reading),
        ],
        ANGLE_ERROR_DEG,
    )
    boundary_polygon = np.vstack(
        [boundary_result.vertices, boundary_result.vertices[0]]
    )
    axes[0].fill(
        boundary_polygon[:, 0],
        boundary_polygon[:, 1],
    ...
    axes[0].plot(
        boundary_polygon[:, 0], boundary_polygon[:, 1], color="#9E3D32", lw=1.8
    )
    axes[0].plot(
        [boundary_result.point_a[0], boundary_result.point_b[0]],
        [boundary_result.point_a[1], boundary_result.point_b[1]],
        color="#D1495B",
        lw=2.0,
        marker="o",
        ms=3.5,
    ...
    axes[0].set_title(
        f"(a) 全精度误差边界场景\n精确直径 {boundary_result.diameter:.3f} m"
    )
    ...
    near_result = solve_localization(
        [
            Measurement(0.0, 0.0, 0.0),
            Measurement(*second, bearing_deg(second, near_source)),
        ],
        ANGLE_ERROR_DEG,
    )
    near_polygon = np.vstack([near_result.vertices, near_result.vertices[0]])
    axes[1].fill(
        near_polygon[:, 0],
        near_polygon[:, 1],
        color="#CDEDE4",
    ...
    axes[1].plot(near_polygon[:, 0], near_polygon[:, 1], color="#245C53", lw=1.8)
    axes[1].scatter(
        [near_source[0]],
        [near_source[1]],
        marker="*",
    ...
    near_check = checks["near_source_triangle"]
    axes[1].set_title(
        "(b) 近距离场景为三角形\n"
        f"精确 {near_check['exact_diameter_m']:.3f} m，一阶 {near_check['first_order_diameter_m']:.3f} m"
    )
    axes[1].set_xlabel("$x$/m")
    axes[1].set_ylabel("$y$/m")
    ...
    comparison = checks["same_nominal_scene_candidate_check"]
    labels = ["闭式点\n35.10°", "安全候选点\n45.00°"]
    values = [
        comparison["current_exact_diameter_m"],
        comparison["forty_five_exact_diameter_m"],
    ]
    bars = axes[2].bar(labels, values, color=["#2F6690", "#A9BCD0"], width=0.58)
    ...
    axes[2].set_title("(c) 同一名义场景的候选点对照\n两者均直线移动1000 m，耗时205 s")
    axes[2].set_ylabel("精确定位直径/m")
    axes[2].set_ylim(118.0, 130.0)
    axes[2].grid(axis="y", alpha=0.2)

    figure.subplots_adjust(wspace=0.30)
    save_figure(figure, "fig4_deterministic_checks")
