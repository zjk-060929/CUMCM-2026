...
# 已知测角误差与位置强相关，移动3--5 m不能产生一条新的有效方位
# 信息。这里不臆造未知的相关长度，而把解析安全第二点的545.206 m
# 位移作为统一设计基线：更短的机会复测只记日志和耗时，不用于收缩
# 定位域。通过门槛也不代表误差统计独立；正确性仍只依赖每次测角
# 误差绝对值不超过1度的集合约束。
MIN_SPATIALLY_DISTINCT_BASELINE_M = SECOND_BASELINE_M
...
class RunSummary:
    ...
    correlated_bearing_ignored_count: int = 0
...
class SearchFirstPolicy:
    ...
    name = "p3_spatial_safe_piggyback_reactive_rolling_dp_v4"
    ...
    def __init__(
        ...
        *,
        ...
        time_selected_supplement: bool = False,
    ):
        self.client = client
        self.logger = logger
        ...
        self.correlated_bearing_ignored_count = 0
        ...
        self.time_selected_supplement = bool(time_selected_supplement)
    ...
    def _direction_has_effective_geometry(
        self, channel: int, point: np.ndarray
    ) -> tuple[bool, float, float]:
        """判断新测点是否具有足够空间基线与交会角。

        这只是拒绝短距离、近共线复测的保守门槛，不把通过门槛的误差
        解释为统计独立。返回值同时供日志记录最近基线与最佳交会角。
        """

        previous = self.observations.get(channel, [])
        if not previous:
        ...
        effective = any(
            baseline >= MIN_SPATIALLY_DISTINCT_BASELINE_M - 1.0e-9
            and separation >= MIN_PIGGYBACK_SEPARATION_DEG - 1.0e-9
            for baseline, separation in zip(baselines, separations)
        )
        return effective, min(baselines), max(separations)
    ...
    def _record_direction(
        ...
        effective, nearest_baseline, best_separation = (
            self._direction_has_effective_geometry(channel, point)
        )
        if not effective:
            self.discovered.add(channel)
            self.correlated_bearing_ignored_count += 1
            self.logger.write(
                "spatially_correlated_bearing_ignored",
                channel=channel,
                position=np.asarray(point, dtype=float).tolist(),
                nearest_prior_baseline_m=nearest_baseline,
                best_geometric_separation_deg=best_separation,
                minimum_baseline_m=MIN_SPATIALLY_DISTINCT_BASELINE_M,
                minimum_separation_deg=MIN_PIGGYBACK_SEPARATION_DEG,
                reason="短基线或近共线复测不用于收缩定位域",
            )
            return None
    ...
    def _collect_shared_observations(
        ...
        candidates = [
            ...
            and self._direction_has_effective_geometry(channel, station)[0]
        ]
        ...
        for channel in candidates:
            ...
            result = response["measure_result"]
            if result == "direction":
                if self._record_direction(channel, station, response) is not None:
    ...
    def _service_channel(
        ...
        if len(observations) >= 2:
            ...
            for attempt_index, candidate_index in enumerate(candidate_order, 1):
                ...
                if self._record_direction(channel, candidate, response) is None:
                    self.logger.write(
                        "second_observation_rejected_as_correlated",
                        channel=channel,
                        attempt_index=attempt_index,
                        candidate_index=candidate_index,
                        position=candidate.tolist(),
        ...
        if self.time_selected_supplement:
        ...
        if supplement is not None:
            ...
            if result == "near":
                ...
                return
            if result == "direction":
                if self._record_direction(channel, supplement, response) is not None:
                    polygon = localization_polygon(
                        observations, self.no_signal_points[channel]
                    )
            else:
    ...
    def _collect_piggyback_observations(
        ...
        *,
        ...
        candidates = [
            ...
            and self._direction_has_effective_geometry(other, point)[0]
        ]
        if self.state.receiver_channel in candidates:
            candidates.remove(self.state.receiver_channel)
            candidates.insert(0, self.state.receiver_channel)
        for channel in candidates:
            ...
            result = response["measure_result"]
            if result == "direction":
                if self._record_direction(channel, point, response) is not None:
    ...
    def _reactive_refinement_after_center_failure(
        ...
        """包围圆中心清除失败后，在该新空间位置补测并重算覆盖路线。

        首次失败本身证明真实源不在中心的20米清除圆内。只有该位置与
        既有测向点满足空间基线和交会角门槛时，direction才加入新的
        正负1度半平面；no_signal则通过同频
    ...
    def run(self) -> RunSummary:
        ...
        summary = RunSummary(
            ...
            correlated_bearing_ignored_count=self.correlated_bearing_ignored_count,
        )
