"""Offline entry for the frozen iteration-6 policy."""
import argparse
...
SELECTED = 'cautious'  # Selected on development cases and frozen before heldout.
...
def main():
    ...
    a=ap.parse_args()
    out=Path(a.output or 'results/mission_cost_demo_'+datetime.now().strftime('%Y%m%d_%H%M%S_%f'))
    ...
    try:
        result=MissionCostPolicy(client,**VARIANTS[SELECTED]).run()
