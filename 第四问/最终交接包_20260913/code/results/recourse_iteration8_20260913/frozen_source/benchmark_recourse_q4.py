"""Iteration-8 finite development and frozen paired validation."""
import argparse
...
from benchmark_service_risk_q4 import NAMES as PRIOR_NAMES
...
ROOT=Path('results/recourse_iteration8_20260913')
NAMES=list(dict.fromkeys(['recourse_strategy.py','benchmark_recourse_q4.py','test_recourse_q4.py','run_recourse_q4.py',*PRIOR_NAMES]))
...
def cases(split):
    if split=='development':
        specs=[('mixed',341000,14),('edge_outward',342000,2),('rotated_cluster',343000,2),('minimum_range',344000,2)]
        scales=(100.,300.)
    else:
        specs=[('mixed',351000,35),('edge_outward',352000,5),('rotated_cluster',353000,5),('minimum_range',354000,5)]
        scales=(100.,300.,900.)
...
def run_one(seed,mode,scale,variant,trace=None):
    ...
    if variant=='previous':policy=ServiceRiskPolicy(client,return_weight=.5,wait_mode='risk')
    elif variant=='reference':policy=GuardedRoutePolicy(client,station_margin=500.,skip_gain=.08)
    else:policy=RecoursePolicy(client,**VARIANTS[variant])
...
def describe_extra(rows):
    a=describe(rows)
    for k in ('rejected_route_candidates','low_gain_measure_skips','maximum_accepted_station_potential_m','service_profile_evaluations','waiting_plan_occurrences','bounded_deferred','active_queries','local_order_changes','local_head_changes','local_candidate_evaluations'):
