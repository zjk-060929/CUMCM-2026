"""Iteration-7 finite development and frozen paired validation."""
import argparse
...
from benchmark_mission_cost_q4 import NAMES as PRIOR_NAMES
...
from service_risk_strategy import ServiceRiskPolicy,VARIANTS


ROOT=Path('results/service_risk_iteration7_20260913')
NAMES=list(dict.fromkeys(['service_risk_strategy.py','benchmark_service_risk_q4.py','test_service_risk_q4.py','run_service_risk_q4.py',*PRIOR_NAMES]))
...
def cases(split):
    if split=='development':
        specs=[('mixed',321000,14),('edge_outward',322000,2),('rotated_cluster',323000,2),('minimum_range',324000,2)]
        scales=(100.,300.)
    else:
        specs=[('mixed',331000,35),('edge_outward',332000,5),('rotated_cluster',333000,5),('minimum_range',334000,5)]
        scales=(100.,300.,900.)
...
def run_one(seed,mode,scale,variant,trace=None):
    ...
    if variant=='previous':policy=MissionCostPolicy(client,travel_discount=0.,detour_weight=.5,effect_blend=.5)
    elif variant=='reference':policy=GuardedRoutePolicy(client,station_margin=500.,skip_gain=.08)
    else:policy=ServiceRiskPolicy(client,**VARIANTS[variant])
...
def describe_extra(rows):
    a=describe(rows)
    for k in ('rejected_route_candidates','low_gain_measure_skips','maximum_accepted_station_potential_m','service_profile_evaluations','waiting_plan_occurrences','bounded_deferred','active_queries'):
