"""Offline entry for the frozen iteration-7 policy."""
import argparse
...
SELECTED = 'riskgate'  # Chosen on development and frozen before heldout.
...
def main():
    ...
    a=ap.parse_args()
    out=Path(a.output or 'results/service_risk_demo_'+datetime.now().strftime('%Y%m%d_%H%M%S_%f'))
    ...
    try:
        result=ServiceRiskPolicy(client,**VARIANTS[SELECTED]).run()
