...
def optimize(model, initial, method='alns', iterations=48, seed=8137, warm=None):
    ...
    if method == 'vnd':
        ...
        for depth in range(min(3, model.n)):
            expanded = []
            for prefix, remaining in beam:
                admissible = [k for k in remaining if k not in model.blocked or not (remaining & model.stations)]
                nearest = sorted(admissible, key=lambda k: model.distance[prefix[-1]][k] if prefix else model.from_start[k])[:6]
            ...
            beam = [(prefix,remaining) for _,prefix,remaining,_ in expanded[:4]]
            stats['iterations'] += 1
    elif method == 'alns':
